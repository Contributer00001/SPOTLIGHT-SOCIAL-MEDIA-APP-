...
export const toggleFollow = mutation({
  ...
  handler: async (ctx, args) => {
      ...
      if (existing) {
          // If the user is already following, unfollow by deleting the follow relationship
          await ctx.db.delete(existing._id);
          await updateFollowCounts(ctx, currentUser._id, args.followingId, false);
      } else {
          // If the user is not following, follow by inserting a new follow relationship
...
async function updateFollowCounts(
  ...
  // Retrieve the follower and following user data from the database
